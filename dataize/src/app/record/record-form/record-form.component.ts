...
export class RecordFormComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.crudService.recordFileNameSubject.subscribe((data: string | any) => {
      this.fileName = data.fileName;
      this.filePath = data.path.split('dataize')[1] + "/";
    ...
    this.crudService.recordFileSubject.subscribe(data => {
      this.items = data;
    })
  }
  ...
  onCreateFileDialog(create: boolean) {
    ...
    dialRef.afterClosed().subscribe(data => {
      if (data) {
        if (create) {
          this.crudService.createFile("record", data.fileName, data.picturesPath[0]);
        } else {
          this.crudService.createFile("record", "", data.picturesPath[0]);
        }
      }
    });
  }
  ...
  onOpenFileDialog() {
    this.crudService.getFileNames('record').pipe(
      map(
        (data: string[] | any) => {
          if (data) {
      ...
      dialRef.afterClosed().subscribe(
        (data: { file: string, action: string }) => {
          if (data.file && data.action === "open") {
            this.crudService.openFile("record", data.file);
  ...
  download(url: string, fileName: string) {
    this.crudService.download(url, 'record', fileName);
  }
